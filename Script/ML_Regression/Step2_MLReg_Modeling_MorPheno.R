...
for (z in 4:ncol(pheno)){
  ...
  ModelVarImp_DF <- matrix(NA,nrow=nrow(FinalVar), ncol=Repeat)
  
  for (i in 1:Repeat){
  print("START Modelling")
  print(paste(z,ThrVal,i))
  ...
  Train.Boruta <- Train[,FinalVar[,1]] %>% bind_cols(Train[,ncol(Train)])
  colnames(Train.Boruta)[ncol(Train.Boruta)] <- Trait
  Test.Boruta <- Test[,FinalVar[,1]]  %>% bind_cols(Test[,ncol(Test)])
...
Best <- ModelPerform_DF[ModelPerform_DF$R2==max(ModelPerform_DF$R2),] %>% rownames()
Worst <- ModelPerform_DF[ModelPerform_DF$R2==min(ModelPerform_DF$R2),] %>% rownames()

#BestModel_Plot
#Data arrangement
BestPlot <- cbind.data.frame(
  read.csv(paste(today,"_",Algo,"/",Trait,"/Test/TestData_Rep",print(Best),".csv", sep=""), row.names=1)[,ncol(Test)],
  read.csv(paste(today,"_",Algo,"/",Trait,"/Prediction/",ThrVal,"/Prediction_Rep",print(Best),".csv", sep=""), row.names=1)
)
...
WorstPlot <- cbind.data.frame(
  ...
  read.csv(paste(today,"_",Algo,"/",Trait,"/Prediction/",ThrVal,"/Prediction_Rep",print(Worst),".csv", sep=""), row.names=1)
)
