...
#scaling
#Red types
TMMnorm.Mean <- TMMnorm.Mean %>%  filter(Color=="Red")
TMMnorm.Mean.s <- scale(TMMnorm.Mean[5:ncol(TMMnorm.Mean)]) %>% 
  as.data.frame() 
rownames(TMMnorm.Mean.s) <-SampleNo[1:36]
...
pdf(file = "kmeans/Fig/FigS2a_ElbowPlot_kmeans_Red.pdf", width=4.0, height=4.0)
...
Light <- c(rep(1,9), rep(2,9), rep(3,9), rep(4,9))

#Heatmap visualization by Complexheatmap
pdf(file ="kmeans/Fig/Fig5b_Heatmap_Exp_RedType_k6.pdf",width=5, height=6)
...
Heatmap(Mat, 
        ...
        left_annotation = rowAnnotation(foo=anno_block(gp=gpar(fill=brewer.pal(6,"Dark2")),
                                                       labels=c(paste("RC",formatC(1:6, width=2, flag="0"),"", sep="")),
                                                       labels_gp=gpar(col="White", fontsize=10))),
        border = TRUE,
        use_raster = FALSE)
...
ClusTable.GO <- left_join(ClusTable %>% select(clust,GeneID), GO, by="GeneID", na_matches = "never") %>% 
  na.omit()
write.csv(ClusTable, paste("kmeans/ClusterInfo/Clus",i,"_Exp_RedType.csv", sep=""))
write.csv(ClusTable.GO, paste("kmeans/ClusterInfo/Clus",i,"_GO_RedType.csv", sep=""), row.names = FALSE)
